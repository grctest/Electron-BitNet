...
function runCommand(command) {
  try {
      (0,child_process__WEBPACK_IMPORTED_MODULE_0__.execSync)(command, { stdio: 'inherit' });
  } catch (error) {
...
function runInference(args) {
  ...
  if (os__WEBPACK_IMPORTED_MODULE_2___default().platform() === 'win32') {
      mainPath = path__WEBPACK_IMPORTED_MODULE_1___default().join(buildDir, 'bin', 'Release', 'llama-cli.exe');
      if (!fs__WEBPACK_IMPORTED_MODULE_4___default().existsSync(mainPath)) {
          mainPath = path__WEBPACK_IMPORTED_MODULE_1___default().join(buildDir, 'bin', 'llama-cli');
      }
  } else {
      mainPath = path__WEBPACK_IMPORTED_MODULE_1___default().join(buildDir, 'bin', 'llama-cli');
  }
